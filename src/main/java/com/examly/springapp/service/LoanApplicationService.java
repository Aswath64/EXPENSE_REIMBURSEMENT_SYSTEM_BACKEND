...
public class LoanApplicationService {
    ...
    public LoanApplication createApplication(LoanApplication application) {
        if (application.getLoanAmount() < 1000 || application.getLoanAmount() > 50000) {
            throw new ValidationException("Reimbursement amount must be between $1,000 and $50,000");
        }
    ...
    public void sendApplicationSubmittedNotifications(LoanApplication app) {
        try {
            // ✅ Email to Applicant
            String applicantSubject = "Reimbursement Application Submitted";
            String applicantBody = "Hi " + app.getFullName() + ",\n\n" +
                    "Your Reimbursement application for " + app.getLoanType().getName() +
            ...
            for (User user : adminsAndAgents) {
                String subject = "New reimbursement Application Submitted";
                String body = "A new reimbursement application has been submitted.\n\n" +
                        "Applicant: " + app.getFullName() + "\n" +
                        "Reimbursement Type: " + app.getLoanType().getName() + "\n" +
